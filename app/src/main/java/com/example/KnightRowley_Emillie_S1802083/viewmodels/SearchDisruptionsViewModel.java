...
public class SearchDisruptionsViewModel  extends ViewModel {
    ...
    public ExpandableListAdapter getELVAdapter(Context sourceContext, String filterText) {
        elvPrepareData(filterText);
        MainAdapter expandableListAdapter = new MainAdapter(sourceContext, groupList, DisruptionCollection);
        return expandableListAdapter;
    }

    private void elvPrepareData(String filterText){
        repo = DisruptionRepository.getInstance();
        disruptions = repo.disruptions;
        ...
        for (Disruption disruption: disruptions){
            ...
            if(disruption.compareByString(filterText)){
                disruptionBrief = disruption.getDisplayBrief();
                disruptionVerbose = disruption.getDisplayVerbose();
